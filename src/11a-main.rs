    ...
    for _ in 0..20 {
        for i in 0..monkeys.len() {
            ...
            for item in &monkey.queue {
                ...
                let v = eval(format!("old = {} ; {} ; new", item, o).as_str());
                let new = v.unwrap().as_int().unwrap() as usize / 3;
                let next_monkey = if new % monkey.divisible_by == 0 {
                    monkey.next_monkey_true
                } else {
                    monkey.next_monkey_false
                };
                println!("{:?}", (next_monkey, new));
                apply.push((next_monkey, new));
            }
            monkey.queue.clear();
